...
export const readDataFile = (
    path: string,
): TaskEither.TaskEither<Error, DataFile | null> =>
    pipe(
        TaskEither.tryCatch(
            () =>
        ...
        TaskEither.chain(
            flow(
                Json.parse,
                Either.mapLeft(
                    () => new Error("Failed to parse json data file"),
                ),
                Either.chain(
                    flow(
                        dataFileType.decode,
...
export const writeDataFile = (path: string) => (data: DataFile) =>
    pipe(
        Json.stringify(data),
        Either.mapLeft(constant(new Error("Failed to stringify the data"))),
        TaskEither.fromEither,
        TaskEither.chain((str) =>
            TaskEither.tryCatch(
                () =>
                ...
                (err) =>
                    new Error(
                        `Failed to write new data to the data file "${path}"\n${err}`,
                    ),
            ),
        ),
        TaskEither.chain((str) =>
