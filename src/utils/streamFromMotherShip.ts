...
export const streamFromMotherShip = <MessageType = string>(
    method: string, 
    path: string, 
    ...
    TaskEither.map(flow(
        ...
        StreamEither.chainEither(data => 
            type.name === "string"
                ? pipe(
                    type.decode(data),
                    Either.mapLeft(() => new Error("Failed to decode mother-ship message"))
                )
                : pipe(
                    Json.parse(data),
                    Either.mapLeft(() => new Error("Failed to parse json response")),
                    Either.chain(flow(
                        type.decode,
                        Either.mapLeft(() => new Error("Failed to decode response"))
                    )),
                )        
        )
    ))
