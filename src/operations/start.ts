...
export const start: Executor<"start"> = (args) => (ctx) =>
    pipe(
        TaskEither.Do,
        TaskEither.bind(
            "configPath",
            flow(
                ...
                TaskEither.chainFirst((path) =>
                    TaskEither.tryCatch(
                        ...
                        (err) =>
                            new Error(
                                `Cannot access config path at "${path}"\n${err}`,
                            ),
                    ),
                ),
            ),
        ...
        TaskEither.chain(({ data, configPath }) =>
            pipe(
                data,
        ...
        TaskEither.chain(() =>
            TaskEither.fromIO(Console.log(chalk.green("Done"))),
        ),
    );
