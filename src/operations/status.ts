...
export const status: Executor<"status"> = () => ctx => pipe(
    ...
    TaskEither.chain((data) => pipe(
        ...
        TaskEither.orElse(
            constant(TaskEither.of<Error, string>(
                chalk.red(`Mother-ship is missing! It should be at port ${data.port} with PID ${data.pid}.\n`) + 
                `It probably went out of control, so try to find and kill the process then run ${chalk.yellow("ghh clean")} to clear an old data.\n` + 
    ...
    TaskEither.fold(
        (err) => 
            err.message === "OFFLINE" 
            ? TaskEither.fromIO(Console.info(chalk.green("Mother-ship is ") + chalk.yellow("OFFLINE")))
