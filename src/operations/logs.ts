...
const readLines = (path: string) => {
    ...
    let index = 0;
    reader.on("line", (text) =>
        StreamEither.next(stream, { index: index++, text }),
    );
...
const tail = (path: string, linesCount: number) =>
    pipe(
        StreamEither.fromTaskEither(lines(path)),
        StreamEither.chain((totalLines) =>
            pipe(
                readLines(path),
                StreamEither.filter(
                    (data) => data.index >= totalLines - linesCount,
                ),
            ),
        ),
    ...
    pipe(
        ...
        StreamEither.tap(() =>
            pipe(
                ...
                StreamEither.tap((line) =>
                    StreamEither.next(stream, line.text),
                ),
                ...
                TaskEither.chain(
                    flow(
                        ...
                        TaskEither.fromOption(
                            () =>
                                new Error("Failed to get last changed string"),
                        ),
                    ),
                ),
...
export const logs: Executor<"logs"> = (arg) => (ctx) =>
    pipe(
        ...
        TaskEither.chain(
            request("get", `/wing/info/`, WingInfoC, { name: arg.name }),
        ),
        TaskEither.chain((data) =>
            pipe(
                ...
                StreamEither.chain((lastIndex) =>
                    watchLinesSince(data.logFilePath, lastIndex),
                ),
